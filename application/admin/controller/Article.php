...
//            return $page;
            $res = Db::name('article')->order('arc_id')->limit(($page - 1) * $limit, $limit)->select();
//            halt($res);
            foreach ($res as $k => $v) {
                //每篇文章下的评论数
...
//            $res['review_count']=Db::name('review')->where('arc_id',);
            $data = ['code' => 0, 'count' => Db::name('article')->count(), 'msg' => '', 'data' => $res];
            return $data;
        }
        return view();
    }
...
//        FileSys
    ...
    {
        if (Request::isPost()) {
            print_r($_POST);var_dump( $files = request()->file('image'));exit();
            $data = input('post.');
            $tags = $data['tags'];
            unset($data['tags']);
//            halt($tags);
...
//        halt($_FILES);
        ...
        $file = \request()->file('image');
        // 移动到框架应用根目录/uploads/ 目录下
        $info = $file->validate(['size' => 150000, 'ext' => 'jpg,png,gif'])->rule('date')->move('../public/static/uploads');
        if ($info) {
            // 成功上传后 获取上传信息
            // 输出 jpg
            echo $info->getExtension();
